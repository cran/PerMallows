...
\paragraph{Ulam}
Algorithms for counting $S_u(n,d)$ and for the u.a.r. generation of permutations of $n$ items at Ulam distance $d$ are included in \pkg{PerMallows}. The sequence $S_u(n,d)$ appears in the OEIS with code A126065.
...
Another way of generating permutations is by ranking the ratings of a set of items. Suppose we have the results of five students in three different tests. An example of such a file is given in 'data.order'. After reading the file, we can get the ranks of each student with the system function \code{order.ratings}.
...
A useful summary of a sample is given by the number of permutations in the sample in which item $i$ appears at position $j$. This is usually denoted as the frequency matrix or first order marginal matrix. The current package supports it via the \code{frequency.mat} function. The parameter \code{perm} can be either a sample of permutations or a single permutation. The frequency matrix of a single permutation is usually denoted permutation matrix. We will illustrate the \code{frequency.mat} function using the well known APA dataset. 
 
The American Psychological Association (APA) dataset includes 15449 ballots of the election for the president in 1980. 
Each voter ranked at least one of the five candidates. Along with this package we distribute the 5738 ballots that ranked the five candidates by the name of \code{data.apa}. This dataset has been largely used in the literature. In particular, one can find in \cite{diaconis1989} a spectral analysis that takes into account the first order marginal of the dataset which can be computed as follows:
\begin{Schunk}
\begin{Sinput}
> data("data.apa")
...
The arguments \code{perm2} and \code{dist.name} can be omitted. In that case, \code{perm2} is assumed to be the identity, that means that the following code computes the Kendall's-$\tau$ distance between \code{pi} and $e$.
\begin{Schunk}
\begin{Sinput}
> distance(perm1 = pi)
\end{Sinput}
...
The \code{str.cycle} function can be used in order to friendly display the cycles .
\begin{Schunk}
\begin{Sinput}
> str.cycle(cycles)
\end{Sinput}
...
Ragarding the process of counting permutations under the Cayley distance, we also include a function to count the number of permutations with $n$ items and \code{num.cycles} cycles. 
\begin{Schunk}
\begin{Sinput}
...
The generation of random permutations at a prescribed distance is supported by the \code{r.dist.d} function, which generates \code{n} permutations of $n$ items at distance \code{dist.value} for a particular metric.
\begin{Schunk}
\begin{Sinput}
...
Regarding the Cayley distance, the \code{r.perms.cycles} function generates uniformly at random permutations of $n$ items with \code{cycles} cycles. 
\begin{Schunk}
\begin{Sinput}
...
Bear in mind that Mallows model (MM) can be used with every metric for permutation considered in this paper, Cayley, Kendall's-$\tau$, Hamming and Ulam. On the other hand, the Generalized Mallows model (GMM) can only be used with Cayley, Kendall's-$\tau$ and Hamming\footnote{Although strictly speaking the GMM can not be used with the Hamming distance, we denote the similar Weighted Hamming Mallows model as GMM for the sake of clarity.}. Remember that for distributions on permutations on \code{perm.length} items the dispersion parameter vector $\boldsymbol \theta$ has \code{perm.length}-1 terms when the distance name is either Cayley or Kendall's-$\tau$, and \code{perm.length} when the distance is Hamming.
...
If the auxiliary files have been generated with the function \code{generate.aux.files}, learning the parameters of the Ulam distance can be done by reading the files instead of computing the necessary data. 
\begin{Schunk}
\begin{Sinput}
...
If the auxiliary files have been generated with the function \code{generate.aux.files}, the distances sampling for the Ulam distance can rely on them. Again, the \code{disk} parameter must be set to \code{TRUE}.
...
\bibitem[{Press \emph{et~al.}(1992)}]{numerical_recipes_c}
